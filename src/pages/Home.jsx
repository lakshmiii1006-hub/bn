    ...
    <div className="bg-[#FCFBFA] text-[#1A1A1A] overflow-hidden selection:bg-pink-100 min-h-screen">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-12">
                ...
                <div className="pt-10 flex flex-col items-center">
                  ...
                  <SignedOut>
                    <SignInButton mode="modal">
                      <button type="button" className="w-full md:w-80 py-6 border-2 border-gray-100 text-gray-500 rounded-2xl font-black uppercase tracking-[0.4em] text-[10px] hover:border-emerald-900 hover:text-emerald-900 transition-all shadow-sm">Sign in to Leave Feedback</button>
                    </SignInButton>
                  </SignedOut>
                </div>
              </form>
      ...
      {showStickyCTA && (
        <motion.div initial={{ y: 100 }} animate={{ y: 0 }} className="fixed bottom-6 right-6 z-50 lg:hidden">
          <SignedIn>
            <Link to="/book">
              <motion.button whileTap={{ scale: 0.95 }} className="bg-emerald-900 text-white px-8 py-4 rounded-full font-bold text-sm tracking-widest uppercase shadow-2xl active:scale-95">Book Now ↓</motion.button>
            </Link>
          </SignedIn>
          <SignedOut>
            <SignInButton mode="modal">
